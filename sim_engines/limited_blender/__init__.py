...
from cellblender.sim_engines import limited_blender
import limited_blender.limited_blender_sim

from cellblender.cellblender_utils import mcell_files_path


print ( "Executing Limited Blender Simulation" )

# Name of this engine to display in the list of choices (Both should be unique within a CellBlender installation)
plug_code = "LIM_BLENDER"
plug_name = "Prototype Blender Python Simulation"
...
def prepare_runs_data_model_full ( data_model, project_dir, data_layout=None ):
  ...
  if output_detail > 0:    print ( "Inside limited_blender.prepare_runs, project_dir=" + project_dir )
  ...
  final_script_path = os.path.join(script_file_path,"limited_blender_sim.py")
  ...
  if not os.path.exists(final_script_path):
      ...
      for sim_seed in range(start,end+1):
          ...
          command_dict = { 'data_model': data_model,
                           "output_detail": parameter_dictionary['Output Detail (0-100)']['val'],
                           "proj_path": project_dir,
                           "seed": sim_seed,
                           "decay_factor": parameter_dictionary['Reaction Factor']['val'],
                           'wd': project_dir
                         }
  ...
  # Call the actual code to run the jobs
  limited_blender.limited_blender_sim.run_jobs ( command_list )

  # Return an empty list since the jobs will already have been run
  return ( [] )
