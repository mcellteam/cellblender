...
output_detail = 0
for arg in sys.argv:
  if output_detail > 10: my_print ( "   " + str(arg) )
  if arg[0:10] == "proj_path=":
    ...
    if output_detail > 0: my_print ( "Unknown argument = " + str(arg) )
if output_detail > 10: my_print ( "\n\n" )

if output_detail > 0: my_print ( "**********************************************" )
if output_detail > 0: my_print ( "*  Limited Pure Python Prototype Simulation  *" )
if output_detail > 0: my_print ( "*        Updated: February 5th, 2017         *" )
if output_detail > 0: my_print ( "**********************************************" )

if output_detail > 10: my_print ( "Arguments: " + str(sys.argv) )
...
my_print ( "Project path = \"%s\", data_model_file_name = \"%s\"" % (proj_path, data_model_full_path) )

##### Read in the data model itself

dm = None
if len(data_model_full_path) > 0:
  if output_detail > 0: my_print ( "Loading data model from file: " + data_model_full_path + " ..." )
  f = open ( data_model_full_path, 'r' )
  pickle_string = f.read()
  f.close()
  dm = pickle.loads ( pickle_string.encode('latin1') )

if output_detail > 0: my_print ( "Done loading CellBlender model." )

if dm is None:
  my_print ( "ERROR: Unable to use data model" )
  sys.exit(1)
...
if output_detail > 0:
  my_print ( "Parameter Dictionary: " + str(par_val_dict) )
  for k in par_val_dict.keys():
    my_print ( "   " + str(k) + ": " + str(par_val_dict[k]) )
...
for m in mols:
  if output_detail > 0: my_print ( "Molecule " + m['mol_name'] + " is a " + m['mol_type'] + " molecule diffusing with " + str(m['diffusion_constant']) )

for r in rels:
  if output_detail > 0: my_print ( "Release " + str(r['quantity']) + " of " + r['molecule'] + " at (" + str(r['location_x']) + "," + str(r['location_y']) + "," + str(r['location_z']) + ")"  )
...
for r in rxns:
  ...
  if output_detail > 0: my_print ( "Reaction: " + r['reactants'] + " " + arrow + " " + r['products'] + "  [" + str(r['fwd_rate']) + bkwd + "]"  )
...
for i in range(iterations+1):
  ...
  if (i % print_every) == 0:
    if output_detail > 0: my_print ( "File = " + viz_file_name )
      ...
      for r in rxns:
        if r['reactants'] == m['mol_name']:
          ...
          if random.random() < (amount_to_remove - num_to_remove):
            num_to_remove += 1
          if output_detail > 50: my_print ( "React " + m['mol_name'] + " with rate = " + str(rate) + ", remove " + str(num_to_remove) + " (should be about " + str(100*fraction_to_remove) + "%)" )
...
if output_detail > 0: my_print ( "Done simulation.\n" );
