...
plug_code = "LIM_PYTHON_OBJ"
plug_name = "Prototype Python (Object)"
...
class engine:
    ...
    def get_progress_message_and_status ( self, stdout_txt ):
      progress_message = "?"
      task_complete = False
      ...
      for i in reversed(stdout_txt.split("\n")):
          if i.startswith("Iteration "):
              ...
              if (last_iter == total_iter) and (total_iter != 0):
                  task_complete = True
              progress_message = "PySimObj: %d%%" % (percent)
              break
      return ( progress_message, task_complete )


    def postprocess_runs ( self, data_model, command_strings ):
