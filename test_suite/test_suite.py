...
class CellBlenderTestSuitePanel(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        """
        row = self.layout.row()
        row.prop(app, "my_bool", text="Bool")
        row = self.layout.row()
        row.prop(app, "my_string", text="String")
        row = self.layout.row()
        row.operator ( "modtst.dialog_operator", text="Open Dialog" )
        """
...
bpy.ops.wm.save_as_mainfile(filepath=path_to_blend, check_existing=False)
...
print ( "Setting Preferences" )
mcell.cellblender_preferences.mcell_binary = path_to_mcell
mcell.cellblender_preferences.mcell_binary_valid = True
mcell.cellblender_preferences.show_sim_runner_options = True
mcell.run_simulation.simulation_run_control = 'COMMAND'
