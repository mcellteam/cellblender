...
class MCELL_OT_PYDICT_add_parameter(bpy.types.Operator):
    bl_idname = "pydict.add_parameter"
    bl_label = "Add Parameter"
    bl_description = "Add a new parameter"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        context.scene.mcell.pydict_parameter_system.add_parameter(context)
        return {'FINISHED'}


class MCELL_OT_PYDICT_remove_parameter(bpy.types.Operator):
    bl_idname = "pydict.remove_parameter"
    bl_label = "Remove Parameter"
    bl_description = "Remove selected parameter"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        status = context.scene.mcell.pydict_parameter_system.remove_active_parameter(context)
        if status != "":
            self.report({'ERROR'}, status)
        return {'FINISHED'}


class MCELL_OT_PYDICT_clear_parameters(bpy.types.Operator):
    bl_idname = "pydict.clear_parameters"
...
class MCELL_OT_PYDICT_gen_parameters(bpy.types.Operator):
    bl_idname = "pydict.gen_parameters"
...
class MCELL_OT_PYDICT_dump_parameters(bpy.types.Operator):
    bl_idname = "pydict.dump_parameters"
...
class MCELL_OT_PYDICT_eval_expr(bpy.types.Operator):
    bl_idname = "pydict.eval_expr"
    bl_label = "Evaluate Expression"
    bl_description = "Evaluate the selected Expression"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        context.scene.mcell.pydict_parameter_system.evaluate_expression(context)
        return {'FINISHED'}

class MCELL_UL_PYDICT_draw_parameter(bpy.types.UIList):
    #@profile('MCELL_UL_PYDICT_draw_parameter.draw_item')
    def draw_item(self, context, layout, data, item, icon, active_data, active_propname, index):
        global global_params
        par = context.scene.mcell.pydict_parameter_system.parameter_list[index]
        par_id = par.par_id
...
def active_pydict_par_index_changed ( self, context ):
...
def update_pydict_parameter_name ( self, context ):
    """ The "self" passed in is a ParametersPropertyGroup object. """
    print ( "Type of self = " + str ( type(self) ) )
    self.update_name ( context )

def update_pydict_parameter_expression ( self, context ):
    """ The "self" passed in is a ParametersPropertyGroup object. """
    print ( "Type of self = " + str ( type(self) ) )
    print ("Parameter value changed")
    self.update_expression ( context )

def update_pydict_parameter_units ( self, context ):
    """ The "self" passed in is a ParametersPropertyGroup object. """
    print ("Parameter units changed")
    self.update_units ( context )

def update_pydict_parameter_desc ( self, context ):
    """ The "self" passed in is a ParametersPropertyGroup object. """
    print ("Parameter description changed")
    self.update_desc ( context )


def update_pydict_parameter_exprlist ( self, context ):
    """ The "self" passed in is a ParametersPropertyGroup object. """
    print ("Parameter expression list changed")



class PyDictParameterSystemPropertyGroup ( bpy.types.PropertyGroup, Expression_Handler ):
    """This is the class that encapsulates a group (or list) of general purpose parameters"""
    ...
    active_par_index = IntProperty(name="Active Parameter", default=0,                                                                  update=active_pydict_par_index_changed)
    active_name  = StringProperty(name="Parameter Name",    default="Par", description="Unique name for this parameter",                update=update_pydict_parameter_name)
    active_expr  = StringProperty(name="Expression",        default="0",   description="Expression to be evaluated for this parameter", update=update_pydict_parameter_expression)
    active_units = StringProperty(name="Units",             default="",    description="Units for this parameter",                      update=update_pydict_parameter_units)
    active_desc  = StringProperty(name="Description",       default="",    description="Description of this parameter",                 update=update_pydict_parameter_desc)
    ...
    def draw_layout (self, context, layout):
        ...
        if not mcell.initialized:
            ...
            if not self.show_python_dict:
                ...
                if len(mcell.pydict_parameter_system.parameter_list) > 0:
                    par_map_item = mcell.pydict_parameter_system.parameter_list[mcell.pydict_parameter_system.active_par_index]
                    layout.prop(mcell.pydict_parameter_system, "last_selected_id")
                    layout.prop(par_map_item, "name")
                    layout.prop(par_map_item, "par_id")

                row = layout.row()
                row.operator("pydict.gen_parameters")
                row.operator("pydict.clear_parameters")
                row = layout.row()
