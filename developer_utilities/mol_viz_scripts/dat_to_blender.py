#############################################################################################
#
# Terminal input: blender -P ~/src4/leo/blender_terminal/dat_to_blender.py -- ~/.../viz_data/seed_00001 <-- where the .dat or .ascii files are located.
# 
# This file reads binary data files that are generated by MCell and updates their display in blender. 
# Color and object geomerty are not currently handled (because the data is not given in the seed_00001 files).
#
#############################################################################################
...
import re
import sys
sys.path.insert(1, '/home/lmckeereid/src4/cellblender')
...
def update_frame_data(scene):
    ...
    # get mol color and geometry from the json file input through the terminal
    '''
    ...
    for dat_file in filepath:
        if re.match(r'(.*\.0*{}\.dat)'.format(frame_num), str(dat_file)):
            ...
            found = True
            # join the input folder string with the data file string
            #dat_file = str(dat_file)    
            #fullpath = os.path.join(sys.argv[4], dat_file.rsplit("\'",2)[1])
            #fullpath = os.path.join(sys.argv[4], file_match.group(0).rsplit("\'",2)[1])
...
# import data model for geometry and color reference. (.json) # BOB WILL KNOW HOW TO DO THIS/WHERE TO LOOK
bpy.app.handlers.frame_change_pre.append(update_frame_data)

# creates breakpoint for debugging purposes within the terminal:
#__import__('code').interact(local={k: v for ns in (globals(), locals()) for k, v in ns.items()})