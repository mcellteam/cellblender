        ...
        if mol_dict:
            ...
            for mol_name in mol_dict.keys():
                ...
                if not mol_shape_mesh:
                    ...
                    bpy.ops.mesh.primitive_cube_add(radius=0.005, location=[0, 0, 0])
                ...
                if not mol_mat:
                    ...
                    if (mcell.mol_viz.color_index > len(mcell.mol_viz.color_list)-1):
                        mcell.mol_viz.color_index = 0
...
#                    print("Molecules read: %d" % (int(tot)))
        ...
        if mol_dict:
            ...
            for mol_name in mol_dict.keys():
                ...
                # Create object to contain the mol_pos_mesh data if needed
                mol_obj = objs.get(mol_name)
                if not mol_obj:
                    mol_obj = objs.new(mol_name, mol_pos_mesh)
                    scn_objs.link(mol_obj)
